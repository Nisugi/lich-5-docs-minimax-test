...
# Proc to handle reconnection logic
# @example Reconnect if needed
...
# Main thread for handling game connection and logic
# @example Starting the main thread
...
@main_thread = Thread.new {
  ...
  # Checks if the login argument is provided
  # @note This is essential for user authentication.
  ...
  # Custom array with limited size
  # @example Creating a limited array
  #   limited_array = LimitedArray.new
  ...
  # Checks if a specific launch file is provided
  # @note This is used for custom launch configurations.
  if @argv_options[:sal]
    begin
  ...
  if @launch_data
    ...
    # Determines if the game is running in platinum mode
    # @return [Boolean] True if platinum mode is active
  ...
  # Handles the case where the frontend is not required
  # @example Running without a frontend
  #   ruby main.rb --without-frontend
  if ARGV.include?('--without-frontend')
    Thread.new {
      #
      # send the login key
